...
async function isTokenContract(provider, address) {
    try {
        const code = await provider.getCode(address);
        if (code.length < MIN_CONTRACT_SIZE * 2 + 2)
            return false;
...
async function processTransaction(provider, txHash, latestBlock, currentBlock) {
    ...
    try {
        ...
        if (!receipt?.contractAddress)
            return null;
        if (processedAddresses.has(receipt.contractAddress))
            return null;
        if (latestBlock - currentBlock < 5)
            return null;
        if (!(await isTokenContract(provider, receipt.contractAddress)))
            return null;
...
async function processBatch(provider, transactions, latestBlock, currentBlock) {
    const limit = pLimit(CONCURRENT_REQUESTS);
    const promises = transactions.map(txHash => limit(() => processTransaction(provider, txHash, latestBlock, currentBlock)));
...
async function scanChain(chain, targetTokens = 50) {
...
async function main() {
    try {
        ...
        if (command === 'scan') {
            ...
            await Promise.all(['ethereum', 'bsc', 'polygon'].map(chain => scanChain(chain, limit)));
        }
        else {
